...
@section('content')
    <section x-data="rajaongkir()" x-init="getProvinces()">
        <div class="mb-2">
            <h4 class="text-muted">Edit Alamat</h4>
        </div>
        <hr>
        @if ($errors->any())
        ...
        <form action="{{ route('alamat.update', $alamat) }}" method="POST">
            ...
            <div class="d-flex flex-column flex-md-row mb-2" style="gap: 10px">
                ...
                <div class="form-group w-100">
                    ...
                    <select name="kota_id" id="kota" class="form-control" x-bind:disabled="!cities_active"
                        ...
                        <option value="{{ $alamat->kota_id }}" selected>{{ $alamat->kota }}</option>
            ...
            <div class="d-flex justify-content-end mt-4">
                <a href="{{ route('profil') }}" class="btn btn-sm btn-outline-warning me-2"><i
                    ...
                    Kembali</a>
                <button type="submit" class="btn btn-sm btn-warning"><i class="mdi mdi-plus"></i> Ubah Alamat</button>
            </div>
        </form>
    </section>
@endsection
    ...
    <script>
        function rajaongkir() {
            return {
                ...
                provinsi: '',
                provinsi_name: '{{ $alamat->provinsi }}',
                cities: [],
                cities_active: false,
                kota: '',
                kota_name: '{{ $alamat->kota }}',
